...
_AGENT_SYSTEM = (
    ...
    "config.js (templates ship them), KEEP and USE them: route cross-feature "
    "communication through window.events, shared state through window.store "
    "(store.save() persists it), touch input through controls.js, EVERY "
    "tunable number through config.js (add knobs, never bury constants), and "
    "leave errors.js loaded first so crashes show on screen.\n"
    "- A brand-new app needs a short README.md (what it is, what each file "
    "does, how to tune it) — the structure gate requires one. Keep it "
