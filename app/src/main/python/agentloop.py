...
_AGENT_SYSTEM = (
    ...
    "load them from a PINNED CDN — a <script src> tag or an ESM import map "
    "(e.g. three.js or Phaser for graphics). NEVER paste a library's source or a "
    "minified bundle into the workspace: reference it by URL so it loads at "
    "runtime and never enters context (the on-device preview has network). "
    "Prefer libraries you know well over obscure ones — the model already knows "
    "their API, so you write only small glue code.\n"
